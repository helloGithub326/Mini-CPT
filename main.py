...
# Load the words.json and store the values in four lists - languageKeys, easyWords, mediumWords, and hardWords - using a procedure, so that it can be called when needed
def loadWords():
...
# Load the user's data using the name of the user as the parameter and store the values in global variables - languageLearning, difficulty, score, position, seenWords, learnedWords, and masteredWords - using a procedure, so that it can be called when needed
def loadData(name):
    # Make variables global so that they can be accessed and modified from anywhere in the code
...
# Save or create the user's data depending on the parameters that are passed to the procedure and store the values in the user's data file and global variables
def saveData(name, newLanguage="", newDifficulty="", newScore=0, newPosition=0, newSeenWords={}, newLearnedWords={}, newMasteredWords={}, create=False):
    ...
    # If the parameters - newLanguage, newDifficulty, newScore, newPosition, newSeenWords, newLearnedWords, or newMasteredWords - are passed to the procedure without the default values, update the newData variable with the new values
    if newDifficulty != "":
    ...
    # If the name of the user that is passed through the parameter is the same as the global variable username (the current user), update the global variables with the new values
    if name == username:
...
def checkPosition(username):
    ...
    # Sort the scores dictionary by the values in descending order by first converting it to a list to actually sort the values and then reconverting it into a dictionary to know the filenames for each value in sortedScoresList
    ...
    # For every filename in the sortedScores dictionary, check if the position in the file is the same as the position in the sortedScoresKeyList and if it isn't, update the position in the file
...
# Translate a list of words using the googletrans's Translator class that was stored as the translator variable and return a dictionary with the original word as the key and the translated word, definition, and similar words as the values
async def translateList(list):
    translatedWords = {}
    translations = await translator.translate([word for word in list], dest=languageLearning, src="en")
    
    # For each translation that is gotten from the googletrans API, store the translated word, definition, and similar words in the translatedWords dictionary with the original word as the key
...
class App(ctk.CTk):
    ...
    def getUser(self, name="", *args):
        ...
        if hasFile:
            ...
            # Store the words in the allWordsTranslated dictionary into the easyWordsTranslated, mediumWordsTranslated, and hardWordsTranslated dictionaries based on the difficulty of the word
            index = 0
    ...
    # Set the language that was given by the user from the loginFrame to the language code using the languageKeys dictionary and save it in the user's file
...
# Show the basic information of the user, such as the language they are learning, the difficulty they are on, their username, the score they have, and the position they are in the leaderboard
class detailsFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent):
        ...
        # Create the languageLabel, difficultyLabel, usernameLabel, scoreLabel, and positionLabel labels and grid them to the detailsFrame grid system
...
class vocabularyWordFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent):
        ...
        # Set the self.easyWordsTranslated, self.mediumWordsTranslated, and self.hardWordsTranslated variables to the ones from the parent (App) class
        ...
        # Create the wordLabel, originalWordLabel, defLabel, similarWordsLabel, nextButton, previousButton, and practiceButton and grid them to the vocabularyWordFrame grid system
    ...
    def newWord(self):
        ...
        try:
            # Keep choosing a new word until we get a word that the user hasn't seen yet or increase the difficulty level of the user if they have seen all the words from their current difficulty level
            repeat = True
            iterations = 0
            while repeat:
    ...
    def nextWord(self):
        ...
        # If the current word is the last one that the user has seen, add the current word to the seenWords dictionary and get a new word
        if currentWordIndex == (len(self.currentWords)-1):
            ...
            # If the current word isn't the last one that the user has seen, update the word to the next word in the currentWords list
    ...
    def previousWord(self):
        ...
        # Get the index of the current word in the currentWords list
        currentWordIndex = self.currentWords.index(self.currentWord)
        # If the current word is the last one the user has seen, add the current word to the seenWords dictionary and update the word to the previous word in the currentWords list
        if currentWordIndex == (len(self.currentWords)-1):
            seenWords[self.currentWord] = {"translated": self.translatedWord, "definition": self.wordDefinition, "similar": self.similarWords}
            self.updateWord(self.currentWords[currentWordIndex-1])
        # If the current word is not the first one, remove the current word from the current words list and update the word to the previous word
    ...
    def changeDifficulty(self):
        ...
        # Change the difficulty level of the user and set the wordList and wordDict to the new values depending on the current values
        if difficulty == "easy":
            difficulty = "medium"
            self.wordList = mediumWords
            self.wordDict = self.mediumWordsTranslated
            ...
            self.wordList = hardWords
            self.wordDict = self.hardWordsTranslated
        # If the user has seen all the hardWords or the difficuly is already set to viewed, set the wordLabel to the message telling the user to go practice the words
        elif difficulty == "hard":
            difficulty = "viewed"
            self.wordLabel.configure(text="YOU HAVE SEEN ALL THE WORDS!\nPRACTICE THEM TO REMEMBER THEM!")
            self.originalWordLabel.configure(text="")
...
class practiceWordFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent, updateUI):
        ...
        # Create the defLabel, option1Button, option2Button, option3Button, option4Button, and learnButton and grid them to the practiceWordFrame grid system
    ...
    def newQuestion(self):
        ...
        # Create a dictionary of the words that the user hasn't learned or mastered yet by checking if they are not in the learnedWords or masteredWords
        possibleWords = {}
        ...
        # If there is at least one word in the possibleWords dictionary, choose a random word from the dictionary and set the correctAnswer and correctAnswerTranslated variables to the word's values
        if len(possibleWords) > 0:
            ...
            # Add words from either the masteredWords, learnedWords, or seenWords lists depending on hoe many words are in each list
        ...
        # If there are no words in the possibleWords dictionary, tell the user that they have mastered all the words they have learned and tell them to go learn more words
        else:
            options = ["", "", "", ""]
    ...
    # Add the current question word to the correct dictionary based on if they learned or mastered it, save their data, and tell the user that they got it correct
    ...
    # Remove the current question word from the learnedWords dictionary if the word is in the user's learnedWords dicitonary, save their data, and tell the user that they got it incorrect and what the correct answer was
